...
# Test script for Ava API functionality
# Usage: ./test_ava_api.sh

BASE_URL="http://localhost:8000"

echo "🧪 Testing Ava API Functionality"
echo "================================="
echo

echo "📋 1. Testing Conversation Endpoint"
echo "------------------------------------"
echo "Request: POST ${BASE_URL}/api/ava/conversation"
curl -X POST "${BASE_URL}/api/ava/conversation" \
  -H "Content-Type: application/json" \
  -d '{"message": "Hello Ava, how are you today?", "user_id": "test-user-123"}' \
  | python3 -m json.tool
echo
echo
...
for msg in "Hi!" "What do you think about creativity?" "Tell me about yourself" "How was your day?"; do
    echo "Message: \"$msg\""
    curl -s -X POST "${BASE_URL}/api/ava/conversation" \
      -H "Content-Type: application/json" \
      -d "{\"message\": \"$msg\"}" \
...
curl -s -X GET "${BASE_URL}/api/ava/state" | python3 -m json.tool
echo

echo "Create mood state:"
curl -s -X POST "${BASE_URL}/api/ava/state" \
  -H "Content-Type: application/json" \
...
curl -s -X GET "${BASE_URL}/api/ava/state/mood" | python3 -m json.tool
echo

echo "Update mood state:"
curl -s -X PUT "${BASE_URL}/api/ava/state/mood" \
  -H "Content-Type: application/json" \
  -d '{"value": "excited"}' \
  | python3 -m json.tool
echo
...
curl -s -X GET "${BASE_URL}/api/ava/state/nonexistent" \
  | python3 -c "import sys, json; data=json.load(sys.stdin); print('❌ Expected error:', data.get('detail', 'No error details'))"
echo
